...
To add the server: `claude mcp add tix -- tix mcp` (installed binary) or `claude mcp add tix -- npx -y @viewengine/tix mcp` (Node 18+).
...
- `tix: command not found`: tell the user; see https://github.com/alexbruf/tix#install (or use `npx @viewengine/tix` with Node 18+).
