...
- Board view (columns from `tix board --json`, `g` toggles `--group`), ticket detail (`tix show --json`), `h/j/k/l`/arrows navigation, `r` refresh, `q`/Ctrl-C quit, `esc`/backspace back from detail.
- **Live filter (`/`):** typed `KEY:VALUE` tokens (the same grammar `tix ls`/`tix board` already document) reload the board on every keystroke via `tix_io::host::CaptureHost`'s soft-error path — a malformed or incomplete token (e.g. mid-typing "clie") shows the error in the status line instead of replacing the board, so the screen doesn't blank out while typing. `Enter` keeps the filter and exits edit mode; `Esc` reverts to the tokens active before the edit and reloads. No fuzzy/substring matching — it is a thin front end on the real filter engine, not a reimplementation (per the "don't reimplement filter logic" scope note above).
- **Mutating actions:** `m` opens a status picker (`j`/`k`, `Enter`) that runs `tix mv <id> <status>`; `e` opens a `KEY=VALUE ...` line that runs `tix set <id> <tokens>`. Both work from the board (on the selected row) or from detail (on the open ticket), then reload the board and, if the mutated ticket's detail was open, refetch it via `tix show` so the view reflects the write exactly. Errors (validation, usage) show in the status line without discarding the current view.
- Gate so far: `cargo build --workspace` and `cargo test --workspace` green. Verified non-interactively against a scratch copy of `tests/fixtures/workspace`: `board client:nova --json` narrows columns correctly, `mv <id> in_progress --json` and `set <id> owner=alex --json` both produce the exact JSON shape `after_mutation`/`draw_detail` expect. The interactive render/input loop itself has **not** been exercised in a real terminal — the sandbox this was built in has no TTY. Run `cargo run -p tix-tui` from a real terminal inside a workspace before trusting the UI/keybindings themselves.
- Not built: `attach`/`detach` (deliverables), `new` (creating tickets), any confirmation prompt before a mutation fires.
- Owner: main session.
